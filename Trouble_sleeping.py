names = ["Aarav", "Priya", "Dev", "Meera", "Kunver"]
scores = [90, 75, 88, 62, 95]
n = len(scores)
print("=== Score Tracker (n = ", n, "players) ===")
for i in range(n):
    print(i + 1, ". ", names[i], " scores ",sep="")
print()
...
for name in names:
    ...
    if name == target:
        break
print("Search for", target, "| steps = ", steps, "| 0(n) = ", n, " - worst case upper bound")
print()
...
for i in range(n):
    for j in range(i + 1, n):
        ...
        if scores[i] + scores[j] == target_sum:
            print(" ", names[i], "+", names[j], "=", scores[i], "+", scores[j], "=", target_sum)
print("Total comparisons :", steps, "| 0(n^2) - drop constants, and keep n^2")
print()
